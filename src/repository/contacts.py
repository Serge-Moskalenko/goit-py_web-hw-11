...
async def create_contact(db: AsyncSession, contact: ContactCreate) -> Contact:
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(status_code=400, detail="Failed to create contact. Integrity error.")
...
async def update_contact(db: AsyncSession, contact_id: int, contact: ContactCreate) -> Contact | None:
    stmt = select(Contact).filter_by(id=contact_id)
    result = await db.execute(stmt)
...
async def delete_contact(db: AsyncSession, contact_id: int) -> dict:
    stmt = select(Contact).filter_by(id=contact_id)
    result = await db.execute(stmt)
    db_contact = result.scalar_one_or_none()
    if db_contact:
...
async def get_contacts_with_upcoming_birthdays(db: AsyncSession, start_date: date, end_date: date) -> List[Contact]:
    stmt = select(Contact).filter(Contact.birthday.between(start_date, end_date))
    result = await db.execute(stmt)
    return result.scalars().all()
